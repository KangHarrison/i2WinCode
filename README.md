...
这是一个解决 **Windows 台式机 + iPhone** 用户痛点的实用小工具。本项目利用 iPhone 的"快捷指令"自动化和 Python Flask 服务端，实现了在同一局域网（LAN）内，将收到的短信验证码安全、不经过公网地实时转发到 **Windows 台式机**上。
...
1.  **运行服务端：** 双击 `sms_receiver.pyw` 文件运行
